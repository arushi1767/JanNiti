...
{context if context else "No specific scheme context found in database. Use general knowledge."}
...
- how_to_apply: Steps to apply in simple language
- deadlines: Any deadlines or application windows (or "No strict deadline" if not applicable)
...
SCHEME 1: {scheme1}
Context: {context1 if context1 else "General knowledge"}

SCHEME 2: {scheme2}
Context: {context2 if context2 else "General knowledge"}
...
{context if context else "No specific scheme found. Answer based on general knowledge about Indian government schemes."}
